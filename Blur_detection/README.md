...
### 3. 데스존(Death Zone) 및 찌꺼기 청소:

- 화면 맨 아래, 양옆 문틀, 복도 끝점 등에서 오랫동안 인식이 안 되는 객체는 '화면 밖으로 나간 것'으로 간주하고 메모리에서 깔끔하게 삭제합니다.

### 4.중복 100% 사살 (몸통-얼굴 겹침 해결):

- 하나의 사람에 두 개의 박스(예: 얼굴, 몸통)가 겹쳤을 때, 더 작은 박스를 강제로 지워버려 블러가 두 개 생기는 현상을 막습니다.

## 🗺️ 2. 전체적인 순서도 (Flowchart)
### 1. [초기 세팅] 라이브러리 로드, YOLO 모델(CUDA) 로드, 폴더 및 저장 경로 생성.

### 2. [영상 읽기] videos 폴더 내의 영상을 하나씩 불러와 프레임 단위로 읽기 시작.

### 3. [YOLO 탐지] model.track()을 이용해 현재 프레임의 사람(객체) 인식 및 고유 ID 부여.
